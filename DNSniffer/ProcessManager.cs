    ...
    class DNLoader
    {
        ...
        { //Only part of a ReadProcessMemory or WriteProcessMemory request was completed 
            try
            {
                ...
                return WriteProcessMemory(DNProcessHandle, BaseAddress + 0x00845666, patchBytes, patchBytes.Length, 0);
            }
            catch (Exception ex)
            {
        ...
        public bool GetXTEAKey()
        {
            try
            {
                ...
                ReadProcessMemory(DNProcessHandle, 0x01ADCE10, buffer, 4, 0);
                var addr = BitConverter.ToInt32(buffer, 0);
                Console.WriteLine(addr);
                var op = ReadProcessMemory(DNProcessHandle, addr, buffer, buffer.Length, 0);
                File.WriteAllText("Key.txt", "0x" + buffer.Select(x => x.ToString("X2")).Aggregate((x, y) => x + ", 0x" + y));
